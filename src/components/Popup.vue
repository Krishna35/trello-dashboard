<template>
  <transition>
    <div v-if="overlay" class="modal">
      ...
      <input :placeholder="currentData.name" v-model="taskName" />
  ...
  methods: {
    saveElement() {
      if (this.taskName === null) {
        this.taskName = this.currentData.name;
      }
      const task = {
    ...
    deleteElement() {
      this.$store.dispatch("deleteTask", this.currentData);
      this.$store.dispatch("toggleOverlay");
    },
  },
};
